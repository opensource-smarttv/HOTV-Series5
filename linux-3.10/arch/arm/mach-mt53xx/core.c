...
#define MAX_SW_TIMESTAMP_SIZE   (512)
...
static void mt53xx_restart(char mode, const char *cmd)
{
	...
	#if defined(CC_CMDLINE_SUPPORT_BOOTREASON)
		#if defined(CONFIG_MACH_MT5890) || defined(CONFIG_ARCH_MT5891)
		__raw_writeb(0x02,PDWNC_VIRT + REG_RW_RESRV5);
		#endif
	#endif
...
unsigned int ssusb_adb_flag;
EXPORT_SYMBOL(ssusb_adb_flag);
static int __init ssusb_adb_check(char *str)
{
    char *s;
    char tmp[5]={0};
    int j;
...
__setup("ssusb_", ssusb_adb_check);
