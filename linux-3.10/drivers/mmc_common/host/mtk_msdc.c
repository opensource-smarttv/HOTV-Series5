...
#define MSDC_SUPPORT_DISABLE_BKOPS  1

#if MSDC_SUPPORT_DISABLE_BKOPS
struct timer_list bkops_timer;	
bool volatile g_msdc_bkops_enable = false;
bool volatile g_hynix_emmc = false;
#endif
...
const EMMC_FLASH_DEV_T _arEMMC_DevInfo1[] =
{
  ...
  {"H26M31001HPR",            0x4A483447, 0x32611101, 0x00000000, 0x00000F0F, 0x1861001c, 0x01000100, 0x00000002, 0x03000d0d, 0x00000002, 0x03060d0f},
  ...
  //support sdr50
  { "KLM4G1FEPD-B031",        0x00344650, 0x44335202, 0x00000000, 0x00000000, 0x10801010, 0x00100010, 0x00000102, 0x0000130F, 0x00000000, 0x030B090C},
  { "THGBMDG5D1LBAIL",        0x00303034, 0x47363002, 0x00000000, 0x00000000, 0x10801010, 0x00100010, 0x00000102, 0x0000130F, 0x00000000, 0x030B090C},
};
#endif
...
const EMMC_FLASH_DEV_T _arEMMC_DevInfo1_Lemurplus[] =
{
   // Device name	        ID1         ID2	    DS26Sample  DS26Delay   HS52Sample  HS52Delay   DDR52Sample DDR52Delay HS200Sample HS200Delay
   //HS200Sample:MSDC_PAD_TUNE0_CMDRRDLY,MSDC_IOCON_RSPL,MSDC_PAD_TUNE0_CMDRDLY,PAD_TUNE_DAT_WRDLY, 
   //HS200Delay :MSDC_IOCON_W_D_SPL,MSDC_DAT_RDDLY0_D0,MSDC_IOCON_R_SMPL,PAD_DAT_RD_RXDLY
   //HS200Common:INT_DAT_LATCH_CK_SEL , MSDC_PB0_CKGEN_MSDC_DLY_SEL
  ...
  {"H26M31001HPR",		0x4A483447, 0x32611101, 0x00000000, 0x00000F0F, 0x00001F00, 0x001F0010, 0x00000002, 0x03000d0d, 0x00000002, 0x03060d0f},
  //support sdr50
  {"KLM4G1FEPD-B031",   0x00344650, 0x44335202, 0x00000000, 0x00000000, 0x10801010, 0x00100010, 0x00000102, 0x0000130F, 0x00000000, 0x030B090C},
  {"THGBMDG5D1LBAIL",   0x00303034, 0x47363002, 0x00000000, 0x00000000, 0x10801010, 0x00100010, 0x00000102, 0x0000130F, 0x00000000, 0x030B090C},
};
...
#if MSDC_SUPPORT_DISABLE_BKOPS
/*
//can not be write, why ?
static void msdc_bkops_enable(int val)
{
    struct msdc_host *host_ctl = mtk_msdc_host[0];
	char ext_csd[512] = {0};

	printk(KERN_ERR "xhb emmc  , bkops val %d \n");

	
	mmc_send_ext_csd(host_ctl->mmc->card, ext_csd);
	printk("==>BKOPS: en %d, start %d, support %d, status %d ,then do\n",ext_csd[163],ext_csd[164],ext_csd[502],ext_csd[246]);

	
	if(!host_ctl || !host_ctl->mmc || !host_ctl->mmc->card)
	{
		printk("[msdc_bkops_enable] there is no card initialized in host[%d], NULL Pointer!\n",0);
		return ;
	}
	
	int ret = mmc_switch(host_ctl->mmc->card, EXT_CSD_CMD_SET_NORMAL,
			   EXT_CSD_BKOPS_EN, val, 0);

   
	if (ret)
	{
		printk(KERN_ERR "[xhb emmc]BKOPS   Failed  ret %d \n",ret);
	}
	else
	{
		printk(KERN_ERR "[xhb emmc]BKOPS   Success  \n");
	}

	
	mmc_send_ext_csd(host_ctl->mmc->card, ext_csd);
	printk("==>BKOPS: en %d, start %d, support %d, status %d \n",ext_csd[163],ext_csd[164],ext_csd[502],ext_csd[246]);

}
*/
static void msdc_bkops_timer(unsigned long arg)
{
	printk(KERN_ERR "[xhb emmc]BKOPS  Timer enter ,enable BKOPS \n");
	//msdc_bkops_enable(1);
	g_msdc_bkops_enable = true;
}
bool msdc_bkops_is_hynix_specific_emmc(void)
{
	//if ((0x4A483447 == _arEMMC_DevInfo1[emmcidx].u4ID1) && (0x32611101 == _arEMMC_DevInfo1[emmcidx].u4ID2))
	if (true == g_hynix_emmc)
	{
		//STOP BKOPS
		//printk(KERN_ERR "[xhb emmc] Hynix emmc, special disable bkops \n");
		g_msdc_bkops_enable = false;

		init_timer(&bkops_timer);	 
		bkops_timer.data = 0;	 
		bkops_timer.function = msdc_bkops_timer;	
		bkops_timer.expires = jiffies + 10 * HZ;	
		add_timer(&bkops_timer);
...
void msdc_pinmux(struct msdc_host *host)
{
    ...
    if (IS_IC_MT5861_ES1())
    {
			...
			sdr_clr_bits(0xF000D600, (0xF<<4)); 	
			sdr_set_bits(0xF000D600, (0xA<<4)); 	   
			sdr_clr_bits(0xF000D610, 0x1<<7);		
			sdr_set_bits(0xF000D610, 0x1<<7);				 
		
			//Local Arbitor open
			sdr_clr_bits(0xF0012200, (0x1<<5)); 
			sdr_set_bits(0xF0012200, (0x1<<5));
			}
			else
			...
			sdr_clr_bits(0xF000D604, 0x3<<26);	   
			sdr_set_bits(0xF000D604, 0x1<<26);				 
		
			//Local Arbitor open	
			sdr_clr_bits(0xF0012200, (0x1));	 
			sdr_set_bits(0xF0012200, (0x1));	
			}
			else
			{
	...
	if (IS_IC_MT5885()) {
		if(host->hw->host_function == MSDC_EMMC)
		{
			...
			//pinmux register d600[7:6], function 2 - DAT4~DAT7		  
			//pinmux register d610[7], function 1 - CLK		 
			sdr_clr_bits(0xF000D600, (0xF<<4) | (0x1<<15));		
			sdr_set_bits(0xF000D600, (0xA<<4) | (0x1<<15)); 	   			 
			//Local Arbitor open
			sdr_clr_bits(0xF0012200, (0x1<<5));	
			sdr_set_bits(0xF0012200, (0x1<<5));
			
			/* Set 0xF0062400:[4]=0, [31]=0 */
			sdr_clr_bits(0xF0062400, (0x1<<4));
			sdr_set_bits(0xF0062400, (0x1<<31));
		}
		else if(host->hw->host_function == MSDC_SD)
		{
			// GPIO Setting
			//pinmux register d604[31:29], function 1 - CMD/CLK/DAT0~DAT3	 
			sdr_clr_bits(0xF000D604, 0x7<<29); 	   
			sdr_set_bits(0xF000D604, 0x1<<29);				 
		...
		if(host->hw->host_function == MSDC_EMMC)
		{
			...
			//pinmux register d600[7:6], function 2 - DAT4~DAT7		  
			//pinmux register d610[7], function 1 - CLK		 
			sdr_clr_bits(0xF000D600, (0xF<<4) | (0x1<<2));		
			sdr_set_bits(0xF000D600, (0xA<<4) | (0x1<<2)); 	   			 
			//Local Arbitor open
			sdr_clr_bits(0xF0012200, (0x1<<5));	
			sdr_set_bits(0xF0012200, (0x1<<5));
		}
...
void msdc_find_dev(u32 *pCID) //emmc only
{
    ...
    if (0x90 == ((pCID[0] & 0xff000000)>> 24))
   {
        g_hynix_emmc = true;
        //printk(KERN_ERR "It's Hynix emmc \n");
   }
    ...
    #if defined(CONFIG_ARCH_MT5882)
		if (IS_IC_MT5885())
			devNum = sizeof(_arEMMC_DevInfo1_Lemurplus)/sizeof(EMMC_FLASH_DEV_T);
		else
	...
	pr_err("id1:%08X id2:%08X\n", ID1(pCID), ID2(pCID));

	for (i = 0; i < devNum; i++) {
		#if defined(CONFIG_ARCH_MT5882)
		if(IS_IC_MT5885()) 
		{
			if((_arEMMC_DevInfo1_Lemurplus[i].u4ID1 == ID1(pCID)) &&
				((_arEMMC_DevInfo1_Lemurplus[i].u4ID2 & idMask) == (ID2(pCID) & idMask)))
				break;
		}
		else
		{
	...
	emmcidx= (i == devNum)?0:i;
		...
		if(IS_IC_MT5885())
		{
			pr_err("[MSDC] eMMC Name: %s\n", _arEMMC_DevInfo1_Lemurplus[emmcidx].acName);
		}
		else
		{
...
void msdc_smaple_delay(struct mmc_host* mmc,u32 flag)
{
	...
	u32	u4Delay = 0;
		...
		if (IS_IC_MT5885()) {
			...
			//sdr_set_bits(MSDC_PAD_TUNE, 0x1 << MSDC_PAD_RXDLY_SEL_SHIFT);
		}
	#endif
    ...
    if(mmc->ios.timing == MMC_TIMING_LEGACY)
    {
        #if defined(CONFIG_ARCH_MT5882) 
		if(IS_IC_MT5885())
			sdr_set_bits(MSDC_PAD_TUNE0, _arEMMC_DevInfo1_Lemurplus[emmcidx].DS26Delay);
		else
			sdr_set_bits(MSDC_PAD_TUNE0, _arEMMC_DevInfo1[emmcidx].DS26Delay);
		#else
			sdr_set_bits(MSDC_PAD_TUNE0, _arEMMC_DevInfo1[emmcidx].DS26Delay);
		#endif
    }
    else if(mmc->ios.timing == MMC_TIMING_MMC_HS)
    {
		...
		#if defined(CONFIG_ARCH_MT5882)
		if (IS_IC_MT5885()) {
			u4Sample = _arEMMC_DevInfo1_Lemurplus[emmcidx].HS52Sample;
			u4Delay = _arEMMC_DevInfo1_Lemurplus[emmcidx].HS52Delay;
			dev_err(mmc_dev(mmc), "[Kernel]Lemurplus SDR50 param 0x%x , 0x%x \n",u4Sample,u4Delay);
			sdr_set_field(MSDC_PATCH_BIT0, MSDC_PB0_INT_DAT_LATCH_CK_SEL, 0);
			sdr_set_field(MSDC_PAD_TUNE0, MSDC_PAD_TUNE0_CLKTXDLY, 0x0);
		} else {
			u4Sample = _arEMMC_DevInfo1[emmcidx].HS52Sample;
			u4Delay = _arEMMC_DevInfo1[emmcidx].HS52Delay;
			dev_err(mmc_dev(mmc), "[Kernel]Capri SDR50 param 0x%x , 0x%x \n",u4Sample,u4Delay);
			sdr_set_field(MSDC_PATCH_BIT0, MSDC_PB0_INT_DAT_LATCH_CK_SEL, 0);
			sdr_set_field(MSDC_PAD_TUNE0, MSDC_PAD_TUNE0_CLKTXDLY, 0x0);
		}
		#else
			u4Sample = _arEMMC_DevInfo1[emmcidx].HS52Sample;
			u4Delay = _arEMMC_DevInfo1[emmcidx].HS52Delay;
			dev_err(mmc_dev(mmc), "[Kernel]Others SDR50 param 0x%x , 0x%x \n",u4Sample,u4Delay);
			sdr_set_field(MSDC_PATCH_BIT0, MSDC_PB0_INT_DAT_LATCH_CK_SEL, 0);
			sdr_set_field(MSDC_PAD_TUNE0, MSDC_PAD_TUNE0_CLKTXDLY, 0x0);
		#endif
			
		printk(KERN_ERR "Kernel using 50MHZ, Param is 0x%x , 0x%x \n",u4Sample,u4Delay);
		/*++++++++++++++++++++++clock part+++++++++++++++++++*/		
		sdr_clr_bits(MSDC_PATCH_BIT0, MSDC_PB0_CKGEN_MSDC_DLY_SEL);//CKGEN_MSDC_DLY_SEL
		sdr_set_bits(MSDC_PATCH_BIT0, ((u4Sample&0x00f00000)>>20)<< CKGEN_MSDC_DLY_SEL_SHIFT);
		  
		/*++++++++++++++++++++++cmd part+++++++++++++++++++*/
		sdr_clr_bits(MSDC_PATCH_BIT1, MSDC_PB1_CMD_RSP_TA_CNTR);//cmd turn around fix 1
		sdr_set_bits(MSDC_PATCH_BIT1, 0x1<<CMD_RSP_TA_CNTR_SHIFT);

		sdr_clr_bits(MSDC_PAD_TUNE0, MSDC_PAD_TUNE0_CMDRRDLY);//cmd resp rx delay
		sdr_set_bits(MSDC_PAD_TUNE0, ((u4Sample&0xff000000)>>24)<<MSDC_PAD_TUNE0_CMDRRDLY_SHIFT); //clock delay 0xc 
		...
		sdr_clr_bits(MSDC_PAD_TUNE0, MSDC_PAD_TUNE0_CMDRDLY);//cmd rx dly
		sdr_set_bits(MSDC_PAD_TUNE0, ((u4Sample&0x0000ff00)>>8)<<MSDC_PAD_TUNE0_CMDRDLY_SHIFT); //clock delay 0xc 
	
		/*++++++++++++++++++++++write part+++++++++++++++++++*/   
		sdr_clr_bits(MSDC_PATCH_BIT1, MSDC_PB1_WRDAT_CRCS_TA_CNTR);//WRDAT_CRCS_TA_CNTR 
		sdr_set_bits(MSDC_PATCH_BIT1, 0x1<<WRDAT_CRCS_TA_CNTR_SHIFT);//write crc status turn aroud fix 1
		
		sdr_clr_bits(MSDC_PAD_TUNE0, MSDC_PAD_TUNE0_DATWRDLY);
		sdr_set_bits(MSDC_PAD_TUNE0, ((u4Sample&0x000000ff))<<MSDC_PAD_TUNE0_DATWRDLY_SHIFT); //

		sdr_clr_bits(MSDC_IOCON, MSDC_IOCON_W_D_SMPL_SEL_SEL); //set bit9 0 , every write use the same smaple
		...
		sdr_clr_bits(MSDC_PAD_TUNE0, MSDC_PAD_TUNE0_DATRRDLY);//read rx delay
		sdr_set_bits(MSDC_PAD_TUNE0, ((u4Delay&0x000000ff))<<MSDC_PAD_TUNE0_DATRRDLY_SHIFT); 	
    }
    else if(mmc->ios.timing == MMC_TIMING_UHS_DDR50)
    {
		#if defined(CONFIG_ARCH_MT5882)
			if (IS_IC_MT5885()) {
				sdr_set_bits(MSDC_PAD_TUNE0, _arEMMC_DevInfo1_Lemurplus[emmcidx].DDR52Delay&0x0000ffff);//PAD_DAT_RD_RXDLY,PAD_DAT_WR_RXDLY
				sdr_clr_bits(MSDC_PATCH_BIT0, _arEMMC_DevInfo1_Lemurplus);//CKGEN_MSDC_DLY_SEL
				sdr_set_bits(MSDC_PATCH_BIT0, ((_arEMMC_DevInfo1_Lemurplus[emmcidx].DDR52Delay&0x00ff0000)>>16)<<10);
			} else {
				sdr_set_bits(MSDC_PAD_TUNE0, _arEMMC_DevInfo1[emmcidx].DDR52Delay&0x0000ffff);//PAD_DAT_RD_RXDLY,PAD_DAT_WR_RXDLY
				sdr_clr_bits(MSDC_PATCH_BIT0, MSDC_PB0_CKGEN_MSDC_DLY_SEL);//CKGEN_MSDC_DLY_SEL
    ...
    else if(mmc->ios.timing == MMC_TIMING_MMC_HS200)
    {
		...
		#if defined(CONFIG_ARCH_MT5882)
		if (IS_IC_MT5885()) {
			u4Sample = _arEMMC_DevInfo1_Lemurplus[emmcidx].HS200Sample;
			u4Delay = _arEMMC_DevInfo1_Lemurplus[emmcidx].HS200Delay;
			dev_err(mmc_dev(mmc), "[Kernel]Lemurplus HS200 param 0x%x , 0x%x \n",u4Sample,u4Delay);
			sdr_set_field(MSDC_PATCH_BIT0, MSDC_PB0_INT_DAT_LATCH_CK_SEL, 0);
			sdr_set_field(MSDC_PAD_TUNE0, MSDC_PAD_TUNE0_CLKTXDLY, 0x0);
		} else {
			u4Sample = _arEMMC_DevInfo1[emmcidx].HS200Sample;
			u4Delay = _arEMMC_DevInfo1[emmcidx].HS200Delay;
			dev_err(mmc_dev(mmc), "[Kernel]Capri HS200 param 0x%x , 0x%x \n",u4Sample,u4Delay);
			sdr_set_field(MSDC_PATCH_BIT0, MSDC_PB0_INT_DAT_LATCH_CK_SEL, 0);
			...
			u4Sample = _arEMMC_DevInfo1[emmcidx].HS200Sample;
			u4Delay = _arEMMC_DevInfo1[emmcidx].HS200Delay;
			dev_err(mmc_dev(mmc), "[Kernel]Others HS200 param 0x%x , 0x%x \n",u4Sample,u4Delay);
			sdr_set_field(MSDC_PATCH_BIT0, MSDC_PB0_INT_DAT_LATCH_CK_SEL, 0);
			sdr_set_field(MSDC_PAD_TUNE0, MSDC_PAD_TUNE0_CLKTXDLY, 0xc);
		#endif		
		printk(KERN_ERR "Kernel using HS200MHZ, Param is 0x%x , 0x%x \n",u4Sample,u4Delay);
		/*++++++++++++++++++++++clock part+++++++++++++++++++*/
		sdr_clr_bits(MSDC_PATCH_BIT0, MSDC_PB0_CKGEN_MSDC_DLY_SEL);//CKGEN_MSDC_DLY_SEL
		sdr_set_bits(MSDC_PATCH_BIT0, ((u4Sample&0x00f00000)>>20)<< CKGEN_MSDC_DLY_SEL_SHIFT);
...
void msdc_sample_edge(struct mmc_host*mmc, u32 flag)
{
    ...
    {
	/*not set IOCON in ett new flow*/
    }
    else if(mmc->ios.timing == MMC_TIMING_UHS_DDR50)
    {
		#if defined(CONFIG_ARCH_MT5882)
        if(IS_IC_MT5885())
			sdr_set_bits(MSDC_IOCON, _arEMMC_DevInfo1_Lemurplus[emmcidx].DDR52Sample & 0xFFFFFF);
		else
...
static int msdc_do_request_async(struct mmc_host*mmc, struct mmc_request*mrq)
{
	...
	if (!is_card_present(host) || host->power_mode == MMC_POWER_OFF) 
	{
		...
		printk(KERN_ERR "card power off %d\n",__LINE__);
        return 0;
    }
...
static void msdc_init_hw(struct msdc_host *host)
{
   ...
   if(host->hw->host_function == MSDC_SD)
    ...
    host->saved_para.pad_tune = (((hw->cmdrrddly & 0x1F) << 22) | ((hw->cmdrddly & 0x1F) << 16) | ((hw->datwrddly & 0x1F) << 0));//sdr_read32(MSDC_PAD_TUNE0);
...
EXPORT_SYMBOL(msdc_bkops_is_hynix_specific_emmc);
EXPORT_SYMBOL(g_msdc_bkops_enable);
