...
#ifdef TEST_INTERVAL
int mgc_setinterval= 13;  // 2>13    1.024s NAKTIMEOUT
module_param(mgc_setinterval, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(mgc_setinterval,"Host printk bulk out number");
#endif
...
extern bool is_musb_suspend;
extern int mtk_usb_suspend(void);

#define USB_HOST_SESSION_TIMEOUT 400
	...
	void *dummy)
{
	...
	if (event == PM_SUSPEND_PREPARE) {
		for(bPortNum=0; bPortNum< MUC_NUM_MAX_CONTROLLER; bPortNum++)
		{
			...
			if((bPortNum == MUSB_GADGET_PORT_NUM)||(bPortNum==MUSB_WIFI_PORT))
				{
		...
		msleep(2000); //wait for finish the usb device disconnect related action. //wifi need 450ms to finish disconnect
...
static int mgc_resume(struct device *dev)
{
	...
	pThis->MGC_pfPhyReset(pController->pBase, pController->pPhyBase) ;
...
int usbresume(struct device *dev)
{
   ...
   #ifdef CONFIG_ARCH_MT8580
    {
     u4Reg = MGC_PHY_Read32(pThis->pPhyBase,0x1c);
     u4Reg |=  0x00004000; 
     MGC_PHY_Write32(pThis->pPhyBase, 0x1c, u4Reg);
     msleep(5);
     u4Reg |=  0x00002000; 
     MGC_PHY_Write32(pThis->pPhyBase, 0x1c, u4Reg);
     msleep(5);
...
static void MGC_InitEnd(MGC_LinuxCd *pThis, uint8_t bEndpoint_Tx_num, uint8_t bEndpoint_Rx_num)
{
	...
	pThis->bMsdEndRxUsedCount = 0;
	pThis->bMsdEndTxUsedCount = 0;
...
static MGC_LinuxLocalEnd *MGC_FindEnd(MGC_LinuxCd *pThis, struct urb *pUrb)
{
	...
	INFO("[USB] Port-%d: New Dev=0x%X, bRemoteEp=%d, proto=%s, wPacketSize=%d.\n", 
		pThis->bPortNum, (uint32_t)dev, usb_pipeendpoint(pUrb->pipe), MGC_DecodeUrbProtocol(pUrb), wPacketSize);
...
void MGC_CompleteUrb(MGC_LinuxCd *pThis, MGC_LinuxLocalEnd *pEnd, struct urb *pUrb)
{
	...
	if(usb_pipecontrol(pUrb->pipe) && ((pDescriptor->idProduct==0x7662)||(pDescriptor->idProduct==0x7632)))
	{
...
static uint8_t MGC_HsDmaControllerIsr(MGC_HsDmaController *pController)
{
	...
	#else //MSD tx/rx share the same rx urb list for double buffer feature
                pEnd = ((pThis->bMsdEndTxNum == bEnd) && (pThis->bMsdEndDoubleBuffer)) ? (&(pThis->aLocalEnd[EPMSD][bEnd])) : 
    ...
    if (bXmt) /* transmit */
    {
        ...
        */
        ...
        if (bEnd)
        {
            ...
            MGC_WriteCsr16(pBase, MGC_O_HDRC_TXCSR, bEnd, MGC_M_TXCSR_CLRDATATOG);
#if 1
		if(mgc_berror && (mgc_resumtcnt < mgc_printnum) && usb_pipebulk(pUrb->pipe)&&(usb_pipein(pUrb->pipe)== 0))
		{
			INFO("[USB %d]Tx pUrb=0x%p, pEnd->bRemoteEnd=%d, length=%d proto=%s\n", mgc_resumtcnt, pUrb, pEnd->bRemoteEnd, pUrb->transfer_buffer_length,MGC_DecodeUrbProtocol(pUrb));
			INFO("dma buffer ad=0x%x, cpu buffer addr=0x%x.\n", (uint32_t)pUrb->transfer_dma, (uint32_t)pUrb->transfer_buffer);
		}
#endif
...
static void MGC_CheckConnect(unsigned long pParam)
{
...
#ifdef CONFIG_USB_DETECT_IMPROVE				  
	  u4Reg = MGC_Read8(pBase,0x630);
	  printk("[USB] check connect fail, disconnect handle ##status=[%x]\n",u4Reg);
	...
	if(pBase == MUSB_BASE1){
     uint32_t u4Reg;
     printk("[USB][BC12]Disable BC1.2**********************************************start 00\n");
     //printk("[USB][BC12]Disable BC1.2\n");
...
static void MGC_ServiceTxAvail(MGC_LinuxCd *pThis, uint8_t bEnd)
{
    ...
    */
	...
	#else
	//If MSD enable bDoublePacketBuffer, Tx/Rx endpoint number must same, and will use the same urb list
    pEnd = ((pThis->bMsdEndTxNum == bEnd) && (pThis->bMsdEndDoubleBuffer)) ? 
...
static void MGC_UnlinkUrb(MGC_LinuxCd *pThis, struct urb *pUrb)
{
	...
	INFO("cur urbUrb=0x%p, dequeue urb=0x%p.\n", pEnd->pCurrentUrb, pUrb);
	if(MGC_GetCurrentUrb(pEnd) != NULL)
	INFO("cur curUrb info: pUrb=0x%p, bReEnd-%d, length=%d proto=%s.\n ", pEnd->pCurrentUrb, usb_pipeendpoint(pEnd->pCurrentUrb->pipe), pEnd->pCurrentUrb->transfer_buffer_length, MGC_DecodeUrbProtocol(pEnd->pCurrentUrb));
	if (MGC_GetCurrentUrb(pEnd)== pUrb)
    {
        ...
        {                
            ...
            {
            		/*
			*20141216 Leon confirm urb not callback if receive NAK, but NAKTIMEOUT (interval=0) disable.
			*USB Rx REQPKT[1] value, if = 1 if always NAK, if receive data, this bit will be cleaned
			*/
			if((pDescriptor->idProduct==0x7662)||(pDescriptor->idProduct==0x7632))
			{
				wCsrVal = MGC_ReadCsr16(pBase, MGC_O_HDRC_RXCSR, pEnd->bEnd);				
				INFO("USB Rx Ep%d CSR = 0x%x.\n", pEnd->bEnd, wCsrVal);
			}
                /* twice in case of double packet buffering */
				if(!pEnd->bDoublePacketBuffer)
				{
			...
			/*
			*20141216 Leon confirm urb not callback.
			*USB Tx TXPKTRDY[0] value, if hw send out request packet, this bit will be clean, otherwise it is 1.
			*/
			if((pDescriptor->idProduct==0x7662)||(pDescriptor->idProduct==0x7632))
			{
				wCsrVal = MGC_ReadCsr16(pBase, MGC_O_HDRC_TXCSR, pEnd->bEnd);				
				INFO("USB Tx Ep%d CSR = 0x%x.\n", pEnd->bEnd, wCsrVal);
			}	
...
static irqreturn_t MUC_Irq(struct usb_hcd *hcd)
{
	...
	else
	{
		...
		printk("[USB]check linestate later!\n");
           ...
           uint32_t u4Reg;
           MU_MB();
           u4Reg = MGC_PHY_Read32(pBase,(uint32_t)0x24);
           MU_MB();
           u4Reg |= 0x7F009100; 
           MGC_PHY_Write32(pBase,(uint32_t)0x24, u4Reg);
           MU_MB();
           //printk("[USB][BC12]MUC_Irq Enable BC1.2\n");
           u4Reg = MGC_PHY_Read32(pBase, M_REG_PHYACR5);
           u4Reg |= 0x1;
           MGC_PHY_Write32(pBase,M_REG_PHYACR5,u4Reg);
        }
#endif		
...
static int MUC_urb_enqueue(struct usb_hcd *hcd, struct urb *pUrb, gfp_t mem_flags)
{
	...
	DBG(1, "usb device disconnect.\n");
		DBG(2, "[USB] MUC_urb_enqueue: pUrb=0x%p, pUrb->hcpriv=%p proto=%s\n", pUrb, pUrb->hcpriv, MGC_DecodeUrbProtocol(pUrb));
		//return -ENODEV;
    }
    spin_lock_irqsave(&pThis->SelectEndLock, flags);
	if(usb_pipecontrol(pUrb->pipe) && ((pDescriptor->idProduct==0x7662)||(pDescriptor->idProduct==0x7632)))
...
static int MUC_urb_dequeue(struct usb_hcd *hcd, struct urb *pUrb, int status)
{
	...
	pBuffer = 
			   (uint8_t*)((pUrb->transfer_buffer_length > MGC_HSDMA_MIN_DMA_LEN) ? 
			   ((uint32_t)pUrb->transfer_dma) : 
			   ((uint32_t)pUrb->transfer_buffer));
	   if(pUrb->hcpriv)
	   {
		   ...
		   DBG(1, "[USB]==>urb=0x%p, lengh=%d, bRemEnd=%d, bEp=%d, buf_addr = 0x%x\n", pUrb, pUrb->transfer_buffer_length, usb_pipeendpoint(pUrb->pipe), pEnd->bEnd, ((uint32_t)pUrb->transfer_dma));
	   }
	   else
		   DBG(1, "[USB]==>urb=0x%p, length=%d, bRemEnd=%d,buf_addr=0x%x.\n", pUrb, pUrb->transfer_buffer_length, usb_pipeendpoint(pUrb->pipe), ((uint32_t)pUrb->transfer_dma));
	   
	   if(usb_pipeendpoint(pUrb->pipe) == 8)
	   {
		   MGC_SelectEnd(pBase, pEnd->bEnd);
	
		   wTxCsrVal = MGC_ReadCsr16(pBase, MGC_O_HDRC_TXCSR, pEnd->bEnd);
		   INFO("ep8 error u16txpktrdy=0x%x", wTxCsrVal);
		   mgc_printnum +=2;
		   mgc_berror = 1;
	   }
	   if(usb_pipecontrol(pUrb->pipe) && ((pDescriptor->idProduct==0x7662)||(pDescriptor->idProduct==0x7632)))
...
static void MUC_endpoint_disable(struct usb_hcd *hcd, struct usb_host_endpoint *hep)
{
    ...
    if (hep && hep->hcpriv)
    {
        ...
        if (pEnd)
        {
			...
			#endif
#else //#ifndef SUPPORT_SHARE_MGC_END_MSD
			if((pEnd->bEnd != 0) && (pEnd->bEnd != pThis->bMsdEndTxNum) && (pEnd->bEnd != pThis->bMsdEndRxNum))
			{
                pEnd->wFifoAddress = MGC_END0_FIFOSIZE; 
                pEnd->wMaxPacketSize = 0;  /* EPx is not occupied */
			...
			else if ((!pEnd->bIsTx) && (pEnd->bEnd == pThis->bMsdEndRxNum))
			{
				...
				if(pThis->bMsdEndRxUsedCount == 0)
				{
					if((pThis->bMsdEndDoubleBuffer == TRUE)&&(pThis->bMsdEndTxUsedCount == 0)) // If double buffer case, need clean rx/tx wMaxPacketSize at the same time
					{
						...
						pEndTx->wMaxPacketSize = 0;
						pThis->bMsdEndTxNum = 0xff;
						pThis->bMsdEndDoubleBuffer = FALSE;
						
						#ifdef MGC_PRE_CHECK_FREE_ENDPOINT
						pEndTx->bIsOccupy = FALSE; 
						#endif
					}
			...
			else if ((pEnd->bIsTx) && (pEnd->bEnd == pThis->bMsdEndTxNum))
			{
...
static int MUC_hcd_probe(struct platform_device *pdev)
{
	...
	{
		case 0:
			pController->pBase=(void *)MUSB_BASE;
			pController->pPhyBase=(void *)((MUSB_BASE + MUSB_PHYBASE)+ (MUSB_PORT0_PHYBASE));
			break;
		case 1:
			pController->pBase=(void *)MUSB_BASE1;
			pController->pPhyBase=(void *)((MUSB_BASE + MUSB_PHYBASE)+ (MUSB_PORT1_PHYBASE));
			break;
		case 2:
			pController->pBase=(void *)MUSB_BASE2;
			pController->pPhyBase=(void *)((MUSB_BASE + MUSB_PHYBASE)+ (MUSB_PORT2_PHYBASE));
			break;
		case 3:
			pController->pBase=(void *)MUSB_BASE3;
			pController->pPhyBase=(void *)((MUSB_BASE + MUSB_PHYBASE)+ (MUSB_PORT3_PHYBASE));
			break;
		default :
			INFO("register error \n");
			break;
	}
	INFO("[USB- %d]MAC register =%p PHY register=%p \n",pdev->id,pController->pBase,pController->pPhyBase);
