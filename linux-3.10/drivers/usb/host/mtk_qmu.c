...
static void  QMU_flush_q(MGC_LinuxCd *pThis, uint8_t bEnd, uint8_t isRx)
{
    ...
    else
    {
        ...
        */
        ...
        dwIntr = (uint32_t)((wIntEnable | ((uint16_t)(1 << bEnd))) << 16);
        dwIntr &= 0xFFFF0000;
...
static int QMU_set_gpd(MGC_LinuxCd *pThis, uint8_t *pDataBuf, uint32_t data_len, uint8_t EP_Num, uint8_t isIOC, uint8_t IsRx)
{
...
{   
    ...
    for (i=0; i<RXQ_NUM; i++)
    {
        ...
        pBuf->start = (uint32_t)dma_map_single(pThis->dev, pBuf->pLogicalBuf, 
...
void QMU_disable(MGC_LinuxCd *pThis, uint8_t EP_Num, uint8_t isRx)
{
    ...
    // Map to read dram for h/w decode.
    pBuf->start = (uint32_t)dma_map_single(pThis->dev, 
...
int QMU_insert_task(MGC_LinuxCd *pThis, uint8_t EP_Num, uint8_t isRx, uint8_t *buf, uint32_t length, uint8_t isIOC)
{
    void __iomem *base = (void __iomem *)pThis->pRegs;
    int status = 0;
    ...
    status = QMU_set_gpd(pThis, buf, length, EP_Num, isIOC, isRx);
...
wait_rx_ioc:
                    ...
                    if (usb_pipeisoc(pUrb->pipe))
                    {
                        ...
                        */
                        ...
                        if (((uint32_t)pUrb->transfer_dma + (uint32_t)d->offset) != 
                             (uint32_t)TGPD_GET_DATA(pRGpd))
                        {
                            INFO("[USB] QMU_IRQ: address compare fail, 0x%08X, 0x%08X.\n", 
                                (uint32_t)(pUrb->transfer_dma + d->offset), 
                                (uint32_t)TGPD_GET_DATA(pRGpd));                           
                            //BUG();
                        }
