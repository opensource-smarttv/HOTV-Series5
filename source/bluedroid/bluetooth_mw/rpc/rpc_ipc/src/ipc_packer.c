    ...
    BLK_DESC_T *    pt_desc)
{
    ...
    RPC_LOG("Pushed ELEM elem:0x%x size (0x%x)\n",
            (unsigned)pv_elem,
            z_elem_size);
                ...
                if(*ppv_chld != NULL)
                {
                    if((pv_elem_chld = _find_elem_by_raw(pv_pack_buff, *ppv_chld)) == NULL)
                    {
                        ...
                        if(pt_desc_chld == NULL)
                        {
                             ...
                             */
                            RPC_LOG("Cannot get desc for (RAW:0x%x)\n", (unsigned)*ppv_chld);
                            pt_entry->z_init_offs = 0;
                        }
                        else
                        {
    ...
    SIZE_T *                    pz_size)
{
    ...
    RPC_LOG("Start packing, BASE: 0x%x, OP: %s\n", (unsigned)pv_pack_buff, ps_op);
    ...
    {
        RPC_LOG("ARG:%u , type:%u, val:0x%08x\n",
        ...
        switch(pt_arg_itr->e_type & ARG_MASK_TYPE)
        {
            ...
            case ARG_TYPE_REF_DESC:
                ...
                if(z_offs != 0)
                {
                    RPC_LOG("Modify ARG (RAW 0x%x) to (OFFSET:0x%x)\n",
                            (unsigned)pt_arg_itr->u.pv_desc,
                            (unsigned)z_offs);
        ...
        switch(pt_arg_itr->e_type & ARG_MASK_TYPE)
        {
            ...
            case ARG_TYPE_REF_BOOL:
                if(pt_arg_ex_desc->pv_raw != NULL)
                {
                    ...
                    RPC_LOG("ARG (OFFSET:0x%x) is translated to (LOCAL:0x%x) \n",
                            (unsigned)pt_arg_itr->u.pv_desc,
        ...
        FOREACH_BLK_ENTRY(pt_desc, pt_entry)
        {
                ...
                ||  (pt_entry->e_type & ARG_MASK_TYPE) == ARG_TYPE_REF_STR )
            {
                if(pt_entry->z_init_offs != 0)
                {
                    ...
                    RPC_LOG("Translated (OFFSET:0x%x)to (LOCAL:0x%x)\n",
                            (unsigned)*ppv_chld,
...
_get_raw_addr(VOID * pv_pack_buff, VOID * pv_data, BLK_DESC_ENTRY_T * pt_entry)
{
        ...
        && (CHAR *)pv_data < (CHAR *)PACK_REF_ELEM_END(pv_pack_buff))
    {
        ...
        if(z_offs == pt_entry->z_init_offs)
        {
            ...
            RPC_ERR(" _get_raw_addr, Not changed ref:0x%x, data:0x%x, z_init_offs:%d\n",
                    (unsigned)pv_raw,
                    (unsigned)pv_data,
        ...
        else
        {
            ...
            RPC_ERR(" _get_raw_addr, Changed ref:0x%x, data:0x%x, z_init_offs:%d\n",
                    (unsigned)pv_raw,
                    (unsigned)pv_data,
                    (unsigned)pt_entry->z_init_offs);
        }
        ...
        if(pt_desc->e_type & ARG_DIR_OUT)
        {
            FOREACH_BLK_ENTRY(pt_desc, pt_entry)
            {
                    ...
                    ||  (pt_entry->e_type & ARG_MASK_TYPE) == ARG_TYPE_REF_STR )
                {
                    ...
                    RPC_LOG("Restored (DATA:0x%x) to (RAW:0x%x)\n",
                ...
                if(pt_entry->e_type == (ARG_TYPE_UINT8 | ARG_DIR_INP))
                {
                    ...
                    if(z_copy_size != 0)
                    {
                        ...
                        *RET_PACK_BLK_REF_SIZE(pv_ret_pack) = z_copy_size;
                        ...
                        RPC_LOG("Split const pv_copy:0x%x, copy_size %d\n", (unsigned)pv_copy, (unsigned)z_copy_size);
                    ...
                    else
                    {
                        ...
                        RPC_LOG("To next const pv_copy:0x%x, pt_entry->z_const_size %d\n", (unsigned)pv_copy, (unsigned)pt_entry->z_const_size);
                    }
            ...
            if(z_copy_size != 0)
            {
                ...
                *RET_PACK_BLK_REF_SIZE(pv_ret_pack) = z_copy_size;
                ...
                pv_ret_data = RET_PACK_BLK_REF_DATA(pv_ret_pack);
                RPC_LOG("End pv_copy:0x%x, copy_size %d\n", (unsigned)pv_copy, (unsigned)z_copy_size);
                memcpy(pv_ret_data, pv_copy, z_copy_size);
...
static VOID _trace_package(VOID * pv_buff)
{
    ...
    RPC_LOG("\n\n");
    RPC_LOG("[IPC MSG _trace_package Base:0x%08x]\n", (unsigned)pv_buff);
    RPC_LOG("+-------------------------------------+\n");
    RPC_LOG(" OP:%s\n", pt_basic->ps_op);
    RPC_LOG(" CB Addr:0x%x\n", (unsigned)pt_basic->pv_cb_addr);
    ...
    PACK_FOR_EACH_ELEM(pv_buff, pv_elem, ui4_i)
    {
       ...
       RPC_LOG(".......................................\n");
       RPC_LOG(" Raw addr:0x%x\n", (unsigned)(ELEM_REF_DESC(pv_elem)->pv_start_raw));
#ifndef IPC_DUMP_DATA
       {
           UCHAR * pc_c;
           for(pc_c = pv_data; (UINT32)(pc_c - (UINT32)((UCHAR*)pv_data + 8)) <= pt_desc->z_size; pc_c += 8)
           {
           ...
           switch(pt_desc->z_size - (pc_c - (UCHAR*)pv_data))
           {
                case 1:
                    ...
                    break;
                case 2:
                    RPC_LOG(" 0x%08x [ %02x %02x                   ]\n",
                        (unsigned)pc_c,
                        (unsigned)pc_c[0],
                        (unsigned)pc_c[1]);
                    break;
                case 3:
                    RPC_LOG(" 0x%08x [ %02x %02x %02x                ]\n",
                        (unsigned)pc_c,
                        (unsigned)pc_c[0],
                        (unsigned)pc_c[1],
                        (unsigned)pc_c[2]);
                    break;
                case 4:
                    RPC_LOG(" 0x%08x [ %02x %02x %02x %02x             ]\n",
                        (unsigned)pc_c,
                        (unsigned)pc_c[0], (unsigned)pc_c[1],
                        (unsigned)pc_c[2], (unsigned)pc_c[3]);
                    break;
                case 5:
                    RPC_LOG(" 0x%08x [ %02x %02x %02x %02x %02x          ]\n",
                        (unsigned)pc_c,
                        (unsigned)pc_c[0], (unsigned)pc_c[1],
                        (unsigned)pc_c[2], (unsigned)pc_c[3],
                        (unsigned)pc_c[4]);
                    break;
                case 6:
                    RPC_LOG(" 0x%08x [ %02x %02x %02x %02x %02x %02x       ]\n",
                        (unsigned)pc_c,
                        (unsigned)pc_c[0], (unsigned)pc_c[1],
                        (unsigned)pc_c[2], (unsigned)pc_c[3],
                    ...
                    break;
                case 7:
                    RPC_LOG(" 0x%08x [ %02x %02x %02x %02x %02x %02x %02x      ]\n",
                        (unsigned)pc_c,
