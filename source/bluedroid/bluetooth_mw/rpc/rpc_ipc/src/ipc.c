...
static VOID _ipc_client_auto_close(VOID * pv_key)
{
    ...
    printf("pv_key_sess=0x%x, t_id is: %ld, thread=0x%x\n", (unsigned int)pv_key_sess, t_id, (unsigned int)rpcu_os_thread_self());
    RPC_ERR("pv_key_sess=0x%x, t_id is: %ld, thread=0x%x\n", (unsigned int)pv_key_sess, t_id, (unsigned int)rpcu_os_thread_self());
...
static IPC_ID_T _get_cur_id()
{
    ...
    RPC_LOG("pv_key_sess=0x%x, t_id is: %ld, thread=0x%x\n", (unsigned int)pv_key_sess, t_id, (unsigned int)rpcu_os_thread_self());
...
static IPC_SESS_T * _get_sess(IPC_ID_T t_id, BOOL b_add_ref, BOOL b_multi_client)
{
    ...
    {
        pt_sess->ui4_refc ++;
        RPC_LOG("[RPCIPC]_get_sess:pt_sess->ui4_refc=%d, ipc thread=0x%x, cur thread=0x%x\n", (int)(pt_sess->ui4_refc), (unsigned int)pt_sess->pv_ipc_thd, (unsigned int)rpcu_os_thread_self());
    }

	if(b_multi_client)
    {
        pt_sess->ui1_multiClientc ++;
        RPC_LOG("[RPCIPC]pt_sess->ui1_multiClientc=%d, ipc thread=0x%x, cur thread=0x%x\n", (int)(pt_sess->ui1_multiClientc), (unsigned int)pt_sess->pv_ipc_thd, (unsigned int)rpcu_os_thread_self());
    }
    rpcu_os_mutex_unlock(pv_tbl_lock);
    return pt_sess;
}
...
static IPC_SESS_T * _get_sess_ex(IPC_ID_T t_id, BOOL b_add_ref, BOOL b_multi_client)
{
    ...
    if (pt_sess->ui4_refc == 0)
    {
        ...
        for(i = 1; (UINT32)i < ui4_max_sess_num;i++)
        {
		  ...
		  {
		    printf("[RPCIPC]_get_sess_ex:find valid pt_sess,t_id =%ld\n",i);
		    pt_sess = pt_tmp_sess;
		    b_other_ct = TRUE;
		    break;
		  }
    ...
    {
        pt_sess->ui1_multiClientc ++;
        RPC_LOG("pt_sess->ui1_multiClientc=%d, ipc thread=0x%x, cur thread=0x%x\n", (int)(pt_sess->ui1_multiClientc), (unsigned int)pt_sess->pv_ipc_thd, (unsigned int)rpcu_os_thread_self());
    }
...
static VOID _unref_sess(IPC_SESS_T * pt_sess)
{
    ...
    RPC_LOG("[RPCIPC]_unref_sess:pt_sess->ui4_refc=%d, ipc thread=0x%x, cur thread=0x%x\n", (int)(pt_sess->ui4_refc), (unsigned int)pt_sess->pv_ipc_thd, (unsigned int)rpcu_os_thread_self());
...
static void  _server_ipc_thd(VOID * pv_arg)
{
    ...
    printf("[RPCIPC]IPC Server thread(threadID=0x%x:t_id:%d)started\n",(unsigned int)rpcu_os_thread_self(), (int)t_id);
    ...
    printf("[RPCIPC]IPC Server Thread(ThreadID=0x%x:t_id:%d) closed\n",(unsigned int)rpcu_os_thread_self(), (int)t_id);
...
static void  _ipc_thd(VOID * pv_arg)
{
    ...
    printf("[RPCIPC]IPC client thread started(ThreadID==0x%x :t_id=%d)\n", (unsigned int)pt_sess->pv_ipc_thd, (int)t_id);
    ...
    printf("[RPCIPC]IPC client thread(ThreadID==0x%x :t_id=%d)closed\n",(unsigned int)rpcu_os_thread_self(), (int)t_id);
...
static INT32 _start_ipc_thd(IPC_ID_T t_id)
{
    ...
    printf("[RPCIPC]<client>App Thread(0x%x) create ipc thread-----\n", (unsigned int)rpcu_os_thread_self());
...
static VOID _ipc_listen_thd(VOID * pv_arg)
{
    ...
    while(1)
    {
        ...
        if(recv(i4_new_sock, recv_buf, sizeof(recv_buf), 0) == sizeof(recv_buf))
        {
	     ...
	     i4_cmd = msg->i4_cmd;
         printf("msg rx from fd: %ld, cmd=%lu, svr_ref=%p, ct_ref=%p\n",
    ...
    ipc_proc_rpc_cb_fct  pf_proc_rpc_cb)
{
    ...
    IPC_CLIENT_T * pt_client;
#endif
    printf("[RPCIPC]<Client>App(threadID=0x%x)call ipc_open_client\n",(unsigned int)rpcu_os_thread_self());
    ...
    if(pt_sess != NULL)
    {
        printf("SN test ipc_open_client(), this thread already has valid session, tid:%ld\n", pt_sess->t_id);
        return pt_sess->t_id;
    }
    #endif
...
INT32 ipc_close_client (IPC_ID_T  t_ipc_id, BOOL bAuto)
{
    ...
    if(pt_sess != NULL)
    {
		...
		else
		{
			...
			pt_sess->ui4_refc -= 1;
			rpcu_os_mutex_unlock(pv_tbl_lock);
			printf("[RPCIPC]<Client>close Ref: %ld, Multiclient:%d\n",pt_sess->ui4_refc, pt_sess->ui1_multiClientc);
		}
...
INT32 ipc_init(UINT32 ui4_max_id, const OS_FNCT_T*  pt_os_fnct)
{
    ...
    printf("ipc_init is called, pv_key_sess=0x%x, L%d\n", (unsigned int)pv_key_sess, __LINE__);
