...
static void btif_media_task_aa_handle_decoder_reset(BT_HDR *p_msg)
{
    ...
    {
        APPL_TRACE_DEBUG("[SNK_AAC] btif_media_task_aa_handle_decoder_reset p_codec_info[%x:%x:%x:%x:%x:%x:%x:%x]",
        ...
        {
            APPL_TRACE_ERROR("[SNK_AAC] ERROR dump_codec_info A2D_ParsSbcInfo fail:%d", a2d_status);
            return;
        }
...
#ifdef USE_AUDIO_TRACK
        APPL_TRACE_DEBUG("%s [USE_AUDIO_TRACK] A2dpSink: aac Create Track", __func__);
...
#else
#if defined(MTK_LINUX_A2DP_AUDIO_TRACK) && (MTK_LINUX_A2DP_AUDIO_TRACK == TRUE)
        APPL_TRACE_EVENT("[SNK_AAC] %s A2dpSink: aac Create Linux Audio Track", __func__);
        btif_media_cb.linux_audio_track =
            BtifAvrcpAudioTrackCreate(btif_a2dp_get_aac_track_frequency(aac_cie.samp_freq),
                                      btif_a2dp_get_aac_track_channel_type(aac_cie.channels));
        if (btif_media_cb.linux_audio_track == NULL) {
            APPL_TRACE_ERROR("[SNK_AAC] %s A2dpSink: Linux Audio Track creation fails!!!", __func__);
            return;
        }
        else
        {
            APPL_TRACE_DEBUG("[SNK_AAC] %s BtifAvrcpAudioTrackCreate OK", __func__);
            return;
        }
#else
        UIPC_Open(UIPC_CH_ID_AV_AUDIO, btif_a2dp_data_cb);
