...
#if defined(MTK_PTS_AV_TEST) && (MTK_PTS_AV_TEST == TRUE)  //tt pts test -- to send Security Control.
...
#if defined(MTK_PTS_AV_TEST) && (MTK_PTS_AV_TEST == TRUE)  //tt pts test --  for AVDTP/SNK/ACP/SIG/SEC/BV-02-C to do Security Conrol response.
...
static void bte_av_media_callback(tBTA_AV_EVT event, tBTA_AV_MEDIA *p_data)
{
    ...
    if (event == BTA_AV_MEDIA_SINK_CFG_EVT) {
        ...
        {
            BTIF_TRACE_DEBUG("[SNK_AAC] bte_av_media_callback. BTA_AV_MEDIA_SINK_CFG_EVT ");
            /* send a command to BT Media Task */
            btif_reset_decoder((UINT8*)(p_data->avk_config.codec_info));
