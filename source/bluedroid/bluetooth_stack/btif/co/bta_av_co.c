...
void bta_av_build_aac_src_cfg (UINT8 *p_pref_cfg, UINT8 *p_src_cap)
{
    ...
    {
        APPL_TRACE_ERROR("<SNK_AAC> src_cap.object_typ:%d", src_cap.object_type);
    }
    ...
    {
        APPL_TRACE_ERROR("<SNK_AAC> src_cap.samp_freq:%d", src_cap.samp_freq);
    }
    ...
    {
        APPL_TRACE_ERROR("<SNK_AAC> src_cap.channels:%d", src_cap.channels);
    }

    if (src_cap.bit_rate_high & A2D_AAC_VBR_MSK == 0)
    {
        APPL_TRACE_WARNING("<SNK_AAC> RMT SRC don't support VBR");
    }
        ...
        UINT8 *p_protect_info)
{
    ...
    if (supported)
    {
        ...
        if (p_peer->num_sup_srcs < BTA_AV_CO_NUM_ELEMENTS(p_peer->srcs))
        {
            ...
            {
                APPL_TRACE_DEBUG("<SNK_AAC> bta_av_audio_sink_getconfig saved caps[%x:%x:%02x:%02x:%02x:%02x:%02x:%02x]",
        ...
        (p_peer->num_sup_srcs == BTA_AV_CO_NUM_ELEMENTS(p_peer->srcs)))
    {
...
#if defined(MTK_A2DP_SNK_AAC_CODEC) && (MTK_A2DP_SNK_AAC_CODEC == TRUE)
        APPL_TRACE_DEBUG("<SNK_AAC> local_supported_SNK_SEPs_num:%d", local_supported_SNK_SEPs_num);
#endif
        ...
        if (bta_av_co_audio_peer_src_supports_codec(p_peer, &index))
        {
            ...
            /* Build the codec configuration for this sink */
            {
                ...
                {
                    APPL_TRACE_DEBUG("<SNK_AAC> to call bta_av_build_aac_src_cfg");
                ...
                if (codec_type == BTA_AV_CODEC_M24)
                {
                    APPL_TRACE_DEBUG("<SNK_AAC> bta_av_audio_sink_getconfig saved caps[%x:%x:%x:%x:%x:%x:%x:%x]",
                            p_codec_info[1], p_codec_info[2], p_codec_info[3],
                            p_codec_info[4], p_codec_info[5], p_codec_info[6],p_codec_info[7],p_codec_info[8]);
                }
                else if (codec_type == BTA_AV_CODEC_SBC)
...
#if defined(MTK_A2DP_SNK_AAC_CODEC) && (MTK_A2DP_SNK_AAC_CODEC == TRUE)
UINT8 GavdpFindRemoteSepByCodecType(tBTA_AV_CO_PEER *p_peer, UINT8 codec_type)
{
    int index;

    if (NULL == p_peer)
    {
        APPL_TRACE_ERROR("[%s] <SNK_AAC> p_peer is NULL.", __FUNCTION__);
        return BTIF_AV_CODEC_NONE;
    }

    APPL_TRACE_DEBUG("[%s] <SNK_AAC> codec_type:%d",__FUNCTION__, codec_type);

    for (index = 0; index < p_peer->num_sup_srcs; index++)
    {
        if (p_peer->srcs[index].codec_type == codec_type)
        {
            APPL_TRACE_DEBUG("[%s] <SNK_AAC> find it, index:%d",__FUNCTION__, index);
            return index;
        }
    }

    APPL_TRACE_ERROR("[%s] <SNK_AAC> index:%d",__FUNCTION__, index);

    return index;
}

UINT8 GavdpFindSepByCodecType(UINT8 codec_type)
{
    int index;

    APPL_TRACE_DEBUG("[%s] <SNK_AAC> codec_type:%d",__FUNCTION__, codec_type);

    for (index = 0; index < local_supported_SNK_SEPs_num; index++)
    {
        if (codec_type == BTIF_AV_CODEC_AAC)
        {
            APPL_TRACE_DEBUG("[%s] <SNK_AAC> local support AAC, index:%d",__FUNCTION__, index);
            return index;
        }
        else if (codec_type == BTIF_AV_CODEC_SBC)
        {
            APPL_TRACE_DEBUG("[%s] <SNK_AAC> local support SBC, index:%d",__FUNCTION__, index);
            return index;
        }
    }
    APPL_TRACE_ERROR("[%s] <SNK_AAC> index:%d",__FUNCTION__, index);
...
UINT8 select_codec(tBTA_AV_CO_PEER *p_peer)
{
    UINT8 codec_type = BTIF_AV_CODEC_NONE;
    int rmt_src_index = 0;
    int local_snk_index = 0;
    ...
    codec_type = BTIF_AV_CODEC_AAC;
#else
    codec_type = BTIF_AV_CODEC_SBC;
#endif

    APPL_TRACE_DEBUG("%s <SNK_AAC> Query codecType = %d", __FUNCTION__, codec_type);
    while ((p_peer->num_sup_srcs == (rmt_src_index = GavdpFindRemoteSepByCodecType(p_peer, codec_type))) ||
           (local_supported_SNK_SEPs_num == (local_snk_index = GavdpFindSepByCodecType(codec_type))))
    {
        if (codec_type == BTIF_AV_CODEC_AAC)
        {
            codec_type = BTIF_AV_CODEC_SBC;
        ...
        {
            APPL_TRACE_ERROR("%s <SNK_AAC> Find none matched SEP codec_type:%d", __FUNCTION__, codec_type);
            break;
        }
        APPL_TRACE_DEBUG("%s <SNK_AAC> Query next codec_type = %d", __FUNCTION__, codec_type);
    }
 ...
 ** Function         bta_av_co_audio_peer_src_supports_codec
 **
 ** Description      input: p_peer,             contain peer SRC supported codec information
 **                                p_src_index,      will be filled with the selected index of SRC
 **
 ** Returns          TRUE if the connection supports this codec, FALSE otherwise
 **
 *******************************************************************************/
...
#if defined(MTK_A2DP_SNK_AAC_CODEC) && (MTK_A2DP_SNK_AAC_CODEC == TRUE)
    bta_av_co_cb.codec_cfg.id = select_codec(p_peer);
#endif
...
#if defined(MTK_A2DP_SNK_AAC_CODEC) && (MTK_A2DP_SNK_AAC_CODEC == TRUE)
            case BTIF_AV_CODEC_AAC:
                APPL_TRACE_ERROR("<SNK_AAC> AAC index:%d", index);
                if (p_src_index)
                {
                    *p_src_index = index;
                }
