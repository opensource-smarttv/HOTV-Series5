...
void bta_av_stream_data_cback(UINT8 handle, BT_HDR *p_pkt, UINT32 time_stamp, UINT8 m_pt)
{
    ...
    {
        /*
        APPL_TRACE_DEBUG("[SNK_AAC] discard AAC stream data");
        osi_free(p_pkt);
        return;
        */
    }
...
#if defined(MTK_A2DP_SNK_AAC_CODEC) && (MTK_A2DP_SNK_AAC_CODEC == TRUE)
        APPL_TRACE_DEBUG("[SNK_AAC][bta_av_setconfig_rsp]p_scb->codec_type:%d", p_scb->codec_type);
...
#if defined(MTK_A2DP_SNK_AAC_CODEC) && (MTK_A2DP_SNK_AAC_CODEC == TRUE)
    APPL_TRACE_DEBUG("<SNK_AAC> bta_av_disc_results -> num_seps:%d, num_snks:%d, num_srcs:%d", p_scb->num_seps, num_snks, num_srcs);
#endif
