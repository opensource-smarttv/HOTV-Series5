...
static void printCodecInfo(tAVDT_SCB *p_scb)
{
    ...
    if (NULL == p_scb)
    {
        AVDT_TRACE_WARNING("[SNK_AAC] printCodecInfo. p_scb is NULL");
    }
    else
    {
        p_cfg = p_scb->cs.cfg;
        AVDT_TRACE_WARNING("[SNK_AAC] printCodecInfo:", p_cfg.codec_info[i]);
        for (i = 0; i < AVDT_CODEC_SIZE; i++)
        {
            AVDT_TRACE_WARNING("[SNK_AAC] %d ", p_cfg.codec_info[i]);
        }
    }
}
#endif
...
#if defined(MTK_A2DP_SNK_AAC_CODEC) && (MTK_A2DP_SNK_AAC_CODEC == TRUE)
            AVDT_TRACE_DEBUG("[SNK_AAC] avdt_ccb_hdl_discover_cmd.(in_use:%s, seid:%d, media_type:%d, tsep:%d)", p_scb->in_use ? "TRUE" : "FALSE", i+1, p_scb->cs.media_type, p_scb->cs.tsep);
#endif
...
#if defined(MTK_A2DP_SNK_AAC_CODEC) && (MTK_A2DP_SNK_AAC_CODEC == TRUE)
    AVDT_TRACE_DEBUG("[SNK_AAC]avdt_ccb_hdl_getcap_cmd. seid:%d", p_data->msg.single.seid);
